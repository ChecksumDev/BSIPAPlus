...
namespace IPA.Injector
{
    internal class AntiPiracy
    {
        public static bool IsInvalid(string path)
        {
            ...
            try
            {
                string userDir = GetPath(new Guid("374DE290-123F-4565-9164-39C4925E467B"),
                    KnownFolderFlags.AliasOnly | KnownFolderFlags.DontVerify);
                string userDir2 = GetPath(new Guid("7d83ee9b-2244-4e70-b1f5-5393042af1e4"),
                    KnownFolderFlags.AliasOnly | KnownFolderFlags.DontVerify);
        ...
        {
            int result =
                SHGetKnownFolderPath(guid, (uint)flags, WindowsIdentity.GetCurrent().Token, out IntPtr outPath);
            if (result < 0)
            {
                return "";
            }
