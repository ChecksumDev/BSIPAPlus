    ...
    public static class Utils
    {
            ...
            Func<Exception, FileInfo, bool>? onCopyException = null)
        {
            ...
            foreach (FileInfo? fi in source.GetFiles())
            {
                ...
                {
                    bool? keepOn = onCopyException?.Invoke(e, fi);
                    if (!keepOn.Unwrap())
                    {
                        throw;
                    }
        ...
        public static DateTime CurrentTime()
        {
            ...
            if (CanUseDateTimeNowSafely)
            {
                return DateTime.Now;
            }

            return DateTime.UtcNow; // return MinValue as a fallback
        }
        ...
        {
            return VersionCompareNoPrerelease(l?.UnderlyingVersion!, r?.UnderlyingVersion!);
        }

        /// <summary>
        ///     Compares a pair of <see cref="Hive.Versioning.Version" />s ignoring both the prerelease and build fields.
        /// </summary>
        /// <param name="l">the left value</param>
        /// <param name="r">the right value</param>
        /// <returns>&lt; 0 if l is less than r, 0 if they are equal in the numeric portion, or &gt; 0 if l is greater than r</returns>
        public static int VersionCompareNoPrerelease(Hive.Versioning.Version l, Hive.Versioning.Version r)
        {
            if (l is null)
            {
            ...
            int cmpVal = l.Major.CompareTo(r.Major);
            if (cmpVal != 0)
            {
                return cmpVal;
            }
        ...
        {
            return new ScopeGuardObject(action);
        }
        ...
        {
            return a.Select(o => $"{o}" /* safer than .ToString() */);
        }
#endif
#if NET3
