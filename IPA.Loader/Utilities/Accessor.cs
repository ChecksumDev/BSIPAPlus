...
namespace IPA.Utilities
{
    ...
    {
        /// <summary>
        ///     A delegate for a field accessor taking a <typeparamref name="T" /> ref and returning a <typeparamref name="U" />
        ///     ref.
        /// </summary>
        ...
        private static Accessor MakeAccessor(string fieldName)
        {
            ...
            string dynMethodName = $"<>_accessor__{fieldName}";
            // unfortunately DynamicMethod doesn't like having a ByRef return type, so reflection it
            DynamicMethod dyn = new(dynMethodName, typeof(U), new[] { typeof(T).MakeByRefType() },
                typeof(FieldAccessor<T, U>), true);
        ...
        {
            return accessors.GetOrAdd(name, MakeAccessor);
        }
        ...
        {
            return ref GetAccessor(name)(ref obj);
        }
    ...
    public static class PropertyAccessor<T, U>
    {
        ...
        private static (Getter, Setter) MakeAccessors(string propName)
        {
            ...
            MethodInfo getM = prop.GetGetMethod(true);
            ...
            else
            {
                if (getM != null)
                {
                    DynamicMethod dyn = new($"<>_get__{propName}", typeof(U), new[] { typeof(T).MakeByRefType() },
                ...
                if (setM != null)
                {
                    DynamicMethod dyn = new($"<>_set__{propName}", typeof(void),
                        new[] { typeof(T).MakeByRefType(), typeof(U) }, typeof(PropertyAccessor<T, U>), true);
                    ILGenerator il = dyn.GetILGenerator();
                    il.Emit(OpCodes.Ldarg_0);
                    il.Emit(OpCodes.Ldind_Ref);
        ...
        {
            return props.GetOrAdd(propName, MakeAccessors);
        }
    ...
    public static class MethodAccessor<T, TDelegate> where TDelegate : Delegate
    {
        ...
        static MethodAccessor()
        {
            ...
            ParameterInfo firstArg = AccessorDelegateInfo<TDelegate>.Parameters.First();
            Type firstType = firstArg.ParameterType;
            ...
            if (typeof(T).IsValueType)
            {
                if (!firstType.IsByRef)
                {
                    throw new InvalidOperationException(
                        "First parameter of a method accessor to a value type is not byref");
                }
            ...
            {
                throw new InvalidOperationException(
                    "First parameter of a method accessor is not assignable to the method owning type");
            }
        }
        ...
        private static TDelegate MakeDelegate(string name)
        {
            ...
            Type retType = AccessorDelegateInfo<TDelegate>.Invoke.ReturnType;
            if (!retType.IsAssignableFrom(method.ReturnType))
            {
                throw new ArgumentException(
                    $"The method found returns a type incompatable with the return type of {typeof(TDelegate)}");
            }
        ...
        ///     if <paramref name="name" /> does not represent the name of a method with the
        ///     given signature
        /// </exception>
        ...
        {
            return methods.GetOrAdd(name, MakeDelegate);
        }
    }
}