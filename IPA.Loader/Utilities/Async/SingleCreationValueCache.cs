...
namespace IPA.Utilities.Async
{
    ...
    public class SingleCreationValueCache<TKey, TValue>
    {
        ...
        private static KeyValuePair<TKey, (ManualResetEventSlim, TValue)> ExpandKeyValuePair(
            KeyValuePair<TKey, TValue> kvp)
        {
            return new KeyValuePair<TKey, (ManualResetEventSlim, TValue)>(kvp.Key, (null, kvp.Value));
        }

        private static KeyValuePair<TKey, TValue> CompressKeyValuePair(
            KeyValuePair<TKey, (ManualResetEventSlim, TValue value)> kvp)
        {
        ...
        {
            return dict.ToArray().Where(k => k.Value.wh == null).Select(CompressKeyValuePair).ToArray();
        }
        ...
        {
            if (dict.TryGetValue(key, out (ManualResetEventSlim wh, TValue val) pair) && pair.wh == null)
            {
                value = pair.val;
                return true;
            }
        ...
        {
            retry:
            if (dict.TryGetValue(key, out (ManualResetEventSlim wh, TValue val) value))
            {
                if (value.wh != null)
                {
                    value.wh.Wait();
            ...
            ManualResetEventSlim wh = new(false);
            (ManualResetEventSlim wh, TValue) cmp = (wh, default);
        ...
        {
            dict = new ConcurrentDictionary<TKey, (ManualResetEventSlim wh, TValue val)>(
                collection.Select(ExpandKeyValuePair));
        }
        ...
        {
            dict = new ConcurrentDictionary<TKey, (ManualResetEventSlim wh, TValue val)>(
                collection.Select(ExpandKeyValuePair), comparer);
        }
