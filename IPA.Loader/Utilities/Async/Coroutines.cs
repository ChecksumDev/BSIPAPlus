...
namespace IPA.Utilities.Async
{
    ...
    public static class Coroutines
    {
        ...
        /// <remarks>
        ///     <para>
        ///         This may be called off of the Unity main thread. If it is, the coroutine start will be scheduled using the
        ///         default
        ///         <see cref="UnityMainThreadTaskScheduler" /> and will be run on the main thread as required by Unity.
        ///     </para>
        ///     <para>
        ///         Unity provides a handful of coroutine helpers that are not <see cref="IEnumerable" />s. Most of these are not
        ///         terribly
        ///         helpful on their own, however <see cref="UnityEngine.WaitForSeconds" /> may be. Instead, prefer to use the
        ///         typical .NET
        ///         <see cref="Task.Wait(TimeSpan)" /> or similar overloads, or use
        ...
        public static Task AsTask(IEnumerator coroutine)
        {
            ...
            {
                return UnityMainThreadTaskScheduler.Factory.StartNew(() => AsTask(coroutine), default, default,
                    UnityMainThreadTaskScheduler.Default).Unwrap();
            }

            TaskCompletionSource<VoidStruct> tcs = new(coroutine, AsTaskSourceOptions);
            _ = PluginComponent.Instance.StartCoroutine(new AsTaskCoroutineExecutor(coroutine, tcs));
            return tcs.Task;
        }

#if NET4
        private static readonly TaskCreationOptions AsTaskSourceOptions =
        ...
        private class AsTaskCoroutineExecutor : IEnumerator
        {
            ...
            public bool MoveNext()
            {
                do
                {
                    ...
                    try
                    {
                        IEnumerator top = enumerators.Peek();
                        if (top.MoveNext())
                        {
