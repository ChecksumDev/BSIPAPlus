...
namespace IPA.Utilities.Async
{
    ...
    public class UnityMainThreadTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        public IEnumerator Coroutine()
        {
            ...
            try
            {
                while (!Cancelling)
                {
                    if (!tasks.IsEmpty)
                    {
                        int yieldAfter = YieldAfterTasks;
                        sw.Start();
                        for (int i = 0;
                             i < yieldAfter && !tasks.IsEmpty
                                            && sw.Elapsed < YieldAfterTime;
                             i++)
                        {
                            QueueItem task;
                            do
                            {
                                ...
                                {
                                    goto exit; // try dequeue, if we can't exit
                                }
        ...
        {
            return tasks.ToArray().Where(q => q.HasTask).Select(q => q.Task).NonNull().ToArray();
        }
        ...
        internal void QueueAction(Action action)
        {
            ...
            tasks.Enqueue(new QueueItem(action));
        }
        ...
        protected override bool TryExecuteTaskInline(Task task, bool taskWasPreviouslyQueued)
        {
            ...
            {
                if (itemTable.TryGetValue(task, out QueueItem? item))
                {
                    if (!item.HasTask)
                    {
                        return false;
        ...
        private class QueueItem : IEquatable<Task>, IEquatable<QueueItem>
        {
            ...
            {
                return other.HasTask == HasTask && Equals(other.Task);
            }

            public bool Equals(Task? other)
            {
                return other is not null && HasTask && other.Equals(Task);
            }
        }
