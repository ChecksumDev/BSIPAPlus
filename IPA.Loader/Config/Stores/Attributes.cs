...
namespace IPA.Config.Stores.Attributes
{
    ...
    /// <summary>
    ///     Causes a field or property in an object being wrapped by <see cref="GeneratedStore.Generated{T}(Config, bool)" />
    ///     to be
    ...
    ///     that would otherwise be nullable (i.e. a reference type or a <see cref="Nullable{T}" /> type) should never be null,
    ///     and the
    ...
    {
        /// <summary>
        ///     Creates a new <see cref="UseConverterAttribute" /> specifying to use the default converter type for the target
        ///     member.
        /// </summary>
        ...
        public UseConverterAttribute(Type converterType)
        {
            ...
            Type? baseT = ConverterType.BaseType;
            ...
            {
                ConverterTargetType = baseT.GetGenericArguments()[0];
            }

            bool implInterface = ConverterType.GetInterfaces().Contains(typeof(IValueConverter));

            if (ConverterTargetType == null && !implInterface)
            {
                throw new ArgumentException("Type is not a value converter!");
            }
        }
