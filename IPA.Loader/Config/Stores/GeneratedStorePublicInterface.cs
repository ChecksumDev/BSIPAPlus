...
namespace IPA.Config.Stores
{
    ...
    public static class GeneratedStore
    {
        ...
        ///         with an <see cref="IgnoreAttribute" /> applied to them are also ignored. Having properties be
        ///         <see langword="virtual" /> is not strictly
        ///         necessary, however it allows the generated type to keep track of changes and lock around them so that the
        ///         config will auto-save.
        ///     </para>
        ///     <para>
        ...
        ///         method <c>Changed()</c>, then that method may be called to artificially signal to the runtime that the content
        ///         of the object
        ///         has changed. That method will also be called after the write locks are released when a property is set anywhere
        ///         in the owning
        ///         tree. This will only be called on the outermost generated object of the config structure, even if the change
        ///         being signaled
        ///         is somewhere deep into the tree.
        ///     </para>
        ///     <para>
        ///         Similarly, <typeparamref name="T" /> can declare a public or protected, <see langword="virtual" />
        ///         method <c>OnReload()</c>, which will be called on the filesystem reader thread after the object has been
        ///         repopulated with new data
        ///         values. It will be called <i>after</i> the write lock for this object is released. This will only be called on
        ///         the outermost generated
        ///         object of the config structure.
        ///     </para>
        ///     <para>
        ///         Similarly, <typeparamref name="T" /> can declare a public or protected, <see langword="virtual" />
        ///         method <c>CopyFrom(ConfigType)</c> (the first parameter is the type it is defined on), which may be called to
        ///         copy the properties from
        ///         another object of its type easily, and more importantly, as only one change. Its body will be executed after
        ///         the values have been copied.
        ///     </para>
        ///     <para>
        ///         Similarly, <typeparamref name="T" /> can declare a public or protected, <see langword="virtual" />
        ///         method <c>ChangeTransaction()</c> returning <see cref="IDisposable" />, which may be called to get an object
        ///         representing a transactional
        ///         change. This may be used to change a lot of properties at once without triggering a save multiple times.
        ///         Ideally, this is used in a
        ///         <see langword="using" /> block or declaration. The <see cref="IDisposable" /> returned from your implementation
        ///         will have its
        ///         <see cref="IDisposable.Dispose" /> called <i>after</i> <c>Changed()</c> is called, but <i>before</i> the write
        ///         lock is released.
        ///         Unless you have a very good reason to use the nested <see cref="IDisposable" />, avoid it.
        ///     </para>
        ///     <para>
        ///         If <typeparamref name="T" /> is marked with <see cref="NotifyPropertyChangesAttribute" />, the resulting object
        ///         will implement
        ///         <see cref="INotifyPropertyChanged" />. Similarly, if <typeparamref name="T" /> implements
        ...
        {
            T? ret = GeneratedStoreImpl.Create<T>();
        ...
        /// <typeparam name="T">the type to wrap</typeparam>
        /// <returns>
        ///     a generated instance of <typeparamref name="T" /> implementing functionality described by
        ///     <see cref="Generated{T}(Config, bool)" />
        /// </returns>
        /// <seealso cref="Generated{T}(Config, bool)" />
        public static T Create<T>() where T : class
        {
            return GeneratedStoreImpl.Create<T>();
        }
    }
}