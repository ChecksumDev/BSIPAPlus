...
namespace IPA.Config.Stores
{
    internal static partial class GeneratedStoreImpl
    {
        private static bool NeedsCorrection(SerializedMemberInfo member)
        {
            ...
            Type? expectType = GetExpectedValueTypeForType(memberType);
        ...
        private static void EmitCorrectMember(ILGenerator il, SerializedMemberInfo member, bool shouldLock,
            bool alwaysNew, LocalAllocator GetLocal,
            Action<ILGenerator> thisobj, Action<ILGenerator> parentobj)
        {
            ...
            if (!convType.IsValueType)
            {
                ...
                MethodInfo? copyFrom = typeof(IGeneratedStore<>).MakeGenericType(convType)
                    .GetMethod(nameof(IGeneratedStore<Config>.CopyFrom));
                Label noCreate = il.DefineLabel();
                using AllocatedLocal valLocal = GetLocal.Allocate(convType);
            ...
            else
            {
                ...
                IEnumerable<SerializedMemberInfo>? structure = ReadObjectMembers(convType);

                using AllocatedLocal valueLocal = GetLocal.Allocate(convType);
        ...
        private static void EmitLoadCorrectStore(ILGenerator il, SerializedMemberInfo member, bool shouldLock,
            bool alwaysNew, LocalAllocator GetLocal,
