...
namespace IPA.Config.Stores
{
    ...
    {
        private static readonly MethodInfo LogErrorMethod =
            typeof(GeneratedStoreImpl).GetMethod(nameof(LogError), BindingFlags.NonPublic | BindingFlags.Static);

        private static readonly MethodInfo LogWarningMethod =
            typeof(GeneratedStoreImpl).GetMethod(nameof(LogWarning), BindingFlags.NonPublic | BindingFlags.Static);

        private static readonly MethodInfo LogWarningExceptionMethod =
            typeof(GeneratedStoreImpl).GetMethod(nameof(LogWarningException),
            ...
            else
            {
                ...
                if (getter is null || getter.IsPrivate)
                {
                    throw new InvalidOperationException(
                        $"Property {member.Name} does not have a getter and is not ignored");
                }
            ...
            Action<ILGenerator> thisobj)
        {
            ...
            else
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Property {member.Name} does not have a setter and is not ignored");
                }
        ...
        private static void EmitLogError(ILGenerator il, string message, bool tailcall = false,
            Action<ILGenerator>? expected = null, Action<ILGenerator>? found = null)
        {
            if (expected == null)
            {
                expected = il => il.Emit(OpCodes.Ldnull);
            }

            if (found == null)
            {
                found = il => il.Emit(OpCodes.Ldnull);
            }
        ...
        private static void EmitNumberConvertTo(ILGenerator il, Type to, Type from)
        {
            ...
            if (to == Decimal_t)
            {
                ...
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromFloat);
                }
                else if (from == typeof(double))
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromDouble);
                }
                else if (from == typeof(long))
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromLong);
                }
                else if (from == typeof(ulong))
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromULong);
                }
                else if (from == typeof(int))
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromInt);
                }
                else if (from == typeof(uint))
                {
                    il.Emit(OpCodes.Newobj, Decimal_FromUInt);
                }
            ...
            else if (from == Decimal_t)
            {
                ...
                {
                    MethodInfo?
                        method = Decimal_t.GetMethod(
                            $"To{to.Name}"); // conveniently, this is the pattern of the to* names
                    il.Emit(OpCodes.Call, method);
                }
            }
            else if (to == typeof(IntPtr))
            {
        ...
        {
            MethodInfo? method = CreateGParent.MakeGenericMethod(childType);
            parentobj(il);
            il.Emit(OpCodes.Call, method);
        }
        ...
        private sealed class LocalAllocator
        {
            ...
            {
                if (!unallocatedLocals.TryGetValue(type, out Stack<LocalBuilder>? list))
                {
            ...
            {
                Stack<LocalBuilder>? list = GetLocalListForType(type);
                if (list.Count < 1)
                {
                    list.Push(ilSource.DeclareLocal(type));
                }
            ...
            public void Deallocate(AllocatedLocal loc)
            {
                ...
                Stack<LocalBuilder>? list = GetLocalListForType(loc.Local.LocalType);
                list.Push(loc.Local);
            }
        }
        ...
        internal static void LogError(Type? expected, Type? found, string message)
        {
            Logger.Config.Notice(
                $"{message}{(expected == null ? "" : $" (expected {expected}, found {found?.ToString() ?? "null"})")}");
        }
