...
namespace IPA.Config.Stores.Converters
{
        ...
        where TCollection : ICollection<T?>
    {
        ...
        ///     <see cref="CollectionConverter{T, TCollection}.CollectionConverter(ValueConverter{T})" />
        ...
        /// <returns>
        ///     a new <typeparamref name="TCollection" /> holding the deserialized content of <paramref name="value" />
        /// </returns>
        /// <seealso cref="ValueConverter{T}.FromValue(Value, object)" />
        public override TCollection FromValue(Value? value, object parent)
        {
            ...
            {
                throw new ArgumentException("Argument not a List", nameof(value));
            }

            TCollection? col = Create(list.Count, parent);
        ...
        {
            return Value.From(obj.Select(t => BaseConverter.ToValue(t, parent)));
        }
    }

    /// <summary>
    ///     A <see cref="CollectionConverter{T, TCollection}" /> which default constructs a converter for use as the value
    ///     converter.
    /// </summary>
    ...
    {
        /// <summary>
        ///     Creates a <see cref="CollectionConverter{T, TCollection}" /> using a default constructed
...
#if NET4
    /// <summary>
    ///     A <see cref="CollectionConverter{T, TCollection}" /> for an <see cref="ISet{T}" />, creating a
    ///     <see cref="HashSet{T}" /> when deserializing.
    /// </summary>
    ...
    /// <summary>
    ///     A <see cref="CollectionConverter{T, TCollection}" /> for an <see cref="IList{T}" />, creating a
    ///     <see cref="List{T}" /> when deserializing.
    /// </summary>
