...
namespace IPA.Config.Data
{
    ...
    ///     Use <see cref="Value.List" /> or
    ///     <see cref="Value.From(System.Collections.Generic.IEnumerable{IPA.Config.Data.Value})" /> to create.
    /// </summary>
    public sealed class List : Value, IList<Value?>
    {
        ...
        /// <summary>
        ///     Copies the <see cref="Value" />s in the <see cref="List" /> to the <see cref="Array" /> in
        ///     <paramref name="array" />.
        /// </summary>
        ...
        {
            return ((IList<Value?>)values).GetEnumerator();
        }
        ...
        {
            return $"[{string.Join(",", this.Select(v => v?.ToString() ?? "null").StrJP())}]";
        }
    }
}