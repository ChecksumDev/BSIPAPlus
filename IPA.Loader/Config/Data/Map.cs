...
{
    /// <summary>
    ///     A ordered map of <see cref="string" /> to <see cref="Value" /> for serialization by an
    ///     <see cref="IConfigProvider" />.
    ///     Use <see cref="Value.Map" /> or
    ///     <see cref="Value.From(System.Collections.Generic.IDictionary{string,IPA.Config.Data.Value})" /> to create.
    /// </summary>
    public sealed class Map : Value, IDictionary<string, Value?>
    {
        ...
        {
            return ((IDictionary<string, Value?>)values).Contains(item);
        }
        ...
        {
            return values.Remove(key) && keyOrder.Remove(key);
        }

        bool ICollection<KeyValuePair<string, Value?>>.Remove(KeyValuePair<string, Value?> item)
        {
            return ((IDictionary<string, Value?>)values).Remove(item) && (keyOrder.Remove(item.Key) || true);
        }
        ...
        /// <returns>
        ///     <see langword="true" /> if the key was found and <paramref name="value" /> set, <see langword="false" />
        ///     otherwise
        /// </returns>
        ...
        {
            return $"{{{string.Join(",", this.Select(p => $"\"{p.Key}\":{p.Value?.ToString() ?? "null"}").StrJP())}}}";
        }
    }
}